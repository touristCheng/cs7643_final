...
class Model(nn.Module):
    def __init__(self, args, size_z=128, global_feature_size=1024):
        ...
        #print("Model save path is: ", self.savepath)
        self.size_z = size_z
    ...
    #def forward(self, x=None, gt=None, is_training=False, mean_feature=None, alpha=None, idx_to_plot=[0,1,2,3,4]):
    def forward(self, x, gt):
        ...
        if is_training:
            ...
            post_1 = self.posterior_infer1(feat)
            #print("Post Inference 1 shape: ", post_1.shape)
            #plot_prob(post_1,self.savepath, "post_1", idx_to_plot)
            o_x = self.posterior_infer2(post_1)
            #plot_prob(o_x,self.savepath, "post_2", idx_to_plot)
            #print("Post Inference 2 shape: ", o_x.shape)
            q_mu, q_std = torch.split(o_x, self.size_z, dim=1)
            q_std = F.softplus(q_std)
            q_distribution = torch.distributions.Normal(q_mu, q_std)
            ...
            #plot_prob(z,self.savepath, "z", idx_to_plot)
            #print("z shape: ", z.shape)

        feat += self.generator(z)
        #print("Feat + generator shape: ", feat.shape)
